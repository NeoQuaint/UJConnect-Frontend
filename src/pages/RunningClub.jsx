  ...
  const posts = [
    {
      ...
      content: 'Saturday run club! 5km through APK campus. All paces welcome',
      media_url: '/run.mp4',
      media_type: 'video',
      likes_count: 245,
      comments_count: 56,
    ...
    },
    {
      ...
      user_name: 'Lerato K.',
      user_dept: 'Marketing',
      content: 'Morning miles with the crew. Best way to start the day',
      media_url: '/run.mp4',
      media_type: 'video',
      likes_count: 189,
      comments_count: 34,
    ...
    },
    {
      ...
      content: 'Week 3 of training. Pace getting better every day',
      media_url: '/run.mp4',
      media_type: 'video',
      likes_count: 156,
      comments_count: 23,
    ...
    },
    {
      ...
      user_name: 'Sipho N.',
      user_dept: 'Finance',
      content: 'Trail run this Sunday at Melville Koppies. Meet at 7AM',
      media_url: '/run.mp4',
      media_type: 'video',
      likes_count: 312,
      comments_count: 78,
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '12px 20px', borderBottom: '1px solid #222', position: 'sticky', top: 0, background: '#000', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <h2 style={{ fontSize: '18px', fontWeight: 700, margin: 0, color: 'white' }}>Running Club</h2>
          ...
          <div key={post.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
            ...
            <div style={{ padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: getFacultyColor(post.user_dept), display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 15, flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: '30px', color: '#888' }}>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}><ShareIcon /></span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 600, background: '#000', borderTop: '1px solid #222', display: 'flex', justifyContent: 'space-around', padding: '8px 0', zIndex: 100, paddingBottom: 'max(8px, env(safe-area-inset-bottom))' }}>
        {tabs.map(tab => { const Icon = tab.icon; return (
          <button key={tab.id} onClick={() => handleTabClick(tab.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, color: '#888', fontFamily: 'inherit' }}>
            <Icon /><span style={{ fontSize: 10 }}>{tab.label}</span>
          </button>
        );})}
      </div>
    </div>
